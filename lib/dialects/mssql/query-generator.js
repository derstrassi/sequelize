...
var QueryGenerator = {
  ...
  upsertQuery: function(tableName, insertValues, updateValues, where, rawAttributes, options) {
    ...
    var query = 'MERGE <%= table %> AS <%= targetTable %> USING(SELECT '
      ...
      , updateKeysQuoted = updateKeys.map(function(key) {
          return this.quoteIdentifier(key);
        }, this)
      , updateValuesEscaped = updateKeys.map(function(key) {
          return this.escape(updateValues[key]);
        }, this);

    // create a virtual source table
    query += updateKeys.map(function(key) {
    ...
    query += ') AS <%= sourceTable %>(' + updateKeysQuoted.join(', ') + ') ON (';
    query += where.$or
    ...
    query += ') WHEN NOT MATCHED BY TARGET THEN INSERT(' + updateKeysQuoted.join(', ');
    query += ') VALUES(' + updateValuesEscaped.join(', ');
