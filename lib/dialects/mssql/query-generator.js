...
var QueryGenerator = {
  ...
  upsertQuery: function(tableName, insertValues, updateValues, where, rawAttributes, options) {
    // heavily inspired by http://stackoverflow.com/a/27545377
    options = options || {};
    Utils._.defaults(options, this.options);

    updateValues = Utils.removeNullValuesFromHash(updateValues, options.omitNull, options);

    var query = 'MERGE <%= table %> AS <%= targetTable %> USING(SELECT '
      , modelAttributeMap = {}
      , insertKeys = Object.keys(insertValues)
      , updateKeys = Object.keys(updateValues)
      , insertKeysQuoted = insertKeys.map(function(key) {
          return this.quoteIdentifier(key);
        }, this);

    if (rawAttributes) {
      Utils._.each(rawAttributes, function(attribute, key) {
    ...
    var modelAttributeKeys = Object.keys(modelAttributeMap)
      , insertModelAttributeKeys = modelAttributeKeys.filter(function(key) {
          return insertKeys.indexOf(key) > -1;
        })
      , updateModelAttributeKeys = modelAttributeKeys.filter(function(key) {
          return updateKeys.indexOf(key) > -1;
        });

    // create a virtual source table
    query += insertKeys.map(function(key) {
      var value = this.escape(insertValues[key]);
      key = this.quoteIdentifier(key);
      return value + ' AS ' + key;
    }, this).join(', ');

    // find a match through all where keys (unique, primary)
    query += ') AS <%= sourceTable %>(' + insertKeysQuoted.join(', ') + ') ON (';
    query += where.$or
      .filter(function(clause) {
        // filter out null-clauses
        return !!clause[Object.keys(clause)[0]];
      })
      .map(function(clause) {
        var key = this.quoteIdentifier(Object.keys(clause)[0]);
        return '<%= targetTable %>.' + key + '=' + '<%= sourceTable %>.' + key;
      }, this).join(' AND ');

    // what to do if a match could NOT be found (INSERT)
    query += ') WHEN NOT MATCHED BY TARGET THEN INSERT(' + insertModelAttributeKeys.map(function(key) {
      return this.quoteIdentifier(key);
    }, this).join(', ');
    query += ') VALUES(' + insertModelAttributeKeys.map(function(key) {
      return this.escape(insertValues[key]);
    }, this).join(', ');

    // what to do if a match was found (UPDATE)
    query += ') WHEN MATCHED THEN UPDATE SET ';
    query += updateModelAttributeKeys.map(function(key) {
      var value = this.escape(updateValues[key]);
      key = this.quoteIdentifier(key);
      return '<%= targetTable %>.' + key + '=' + value;
    }, this).join(', ');
    ...
    return Utils._.template(query)({
      table: this.quoteTable(tableName),
      targetTable: this.quoteTable('Target'),
      sourceTable: this.quoteTable('Source')
    });
  },
